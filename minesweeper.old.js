...
function updateCounters(counter, value) {
  ...
  document.getElementById(counter + "_ones").src = `assets/${
    digits[value % 10]
  }.png`;
  document.getElementById(counter + "_tenths").src = `assets/${
    digits[Math.floor(value / 10) % 10]
  }.png`;
  document.getElementById(counter + "_hundreds").src = `assets/${
...
function createField(clicked_x = -1, clicked_y = -1) {
  ...
  if ((difficulty = getQueryVaraible("difficulty"))) {
    ...
    // or get custom field properties
...
function revealCell(y, x) {
  ...
  if (document.getElementById(`_${y}_${x}`).classList.contains("flagged"))
    return; // Can't click open a flagged cell.

  // Generates new field where the clicked position is no mine.
  if (!generated_with_clicked) createField(x, y);

  if (document.getElementById(`_${y}_${x}`).classList.contains("clicked"))
    return; // Check if cell is already revealed
  ...
  if (document.getElementById(`_${y}_${x}`).classList.contains("hoverable"))
    document.getElementById(`_${y}_${x}`).classList.remove("hoverable");
...
function placeFlag(caller) {
  ...
  if (caller.classList.contains("flagged")) {
    ...
    if (!caller.classList.contains("hoverable"))
      caller.classList.add("hoverable");
    flags++;
  } else {
    caller.classList.add("flagged");
    if (caller.classList.contains("hoverable"))
      caller.classList.remove("hoverable");
    flags--;
  }
  updateCounters("flag", flags);
...
function displayHighscores() {
  ...
  document.getElementById("highscores-beginner-score").innerHTML =
    highscores.beginner.score;
  document.getElementById("highscores-beginner-name").innerHTML =
    highscores.beginner.name;
  document.getElementById("highscores-intermediate-score").innerHTML =
    highscores.intermediate.score;
  document.getElementById("highscores-intermediate-name").innerHTML =
    highscores.intermediate.name;
  document.getElementById("highscores-expert-score").innerHTML =
    highscores.expert.score;
  document.getElementById("highscores-expert-name").innerHTML =
    highscores.expert.name;
}
